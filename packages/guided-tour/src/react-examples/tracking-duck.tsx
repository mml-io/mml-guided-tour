...
export const TrackingDuck = memo(({ x, y, z, sx, sy, sz, ry, visibleTo }: TrackingDuckProps) => {
  const duckModelURL = "/assets/guidedtour/duck.glb";

  const positionProbeRef = useRef<MPositionProbeElement | null>(null);
  const duckRef = useRef<MModelElement | null>(null);
  const [updateInterval, setUpdateInterval] = useState<NodeJS.Timeout | null>(null);

  const [rotationY, setRotationY] = useState<number>(0);
  const [rotationZ, setRotationZ] = useState<number>(0);

  const players = useMemo(() => new Map(), []);

  const handleProbeEnterEvent = useCallback(
  ...
  const handleProbeMoveEvent = useCallback(
  ...
  const handleProbeLeaveEvent = useCallback(
  ...
  const handleWindowDisconnectedEvent = useCallback(() => {
